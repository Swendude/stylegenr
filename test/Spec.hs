...
import Stylegen (buildDict)
import Test.Hspec
import TextCleaning (cleanText, removePunct, replaceWhitespace, splitWords, strip)

main :: IO ()
main = hspec $ do
  describe "Test if TextCleaning works" $ do
  ...
  describe "Test if Stylegen works" $ do
    describe "Test if buildDict works" $ do
      it "BuildDict creates a proper styledict" $ do
        buildDict ["thanks", "for", "all", "the", "for", "fish"]
          `shouldBe` fromList
            ( [ ("thanks", ["for"]),
                ("for", ["all", "fish"]),
                ("all", ["the"]),
